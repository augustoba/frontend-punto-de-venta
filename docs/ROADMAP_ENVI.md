...
- **Fix A2** (2026-09-18): `panel-theme.css` quedó dentro de `@layer components`. Antes eran reglas sin capa y le ganaban a las utilidades de Tailwind (`hidden`, `sm:hidden`, `sm:flex`), por eso en desktop se veía duplicada la barra superior de mobile. Verificado en el navegador (`/admin`, sesión de superadmin ya iniciada en ese Chrome).
- **Observado en P0 (a resolver, NO tocado):** el panel sigue mostrando datos del ecommerce porque **están guardados en la base `puntoventa`** (nombre "Estilos Pequeños", 10 productos de ropa con talles, "Por reponer 53") y porque el menú aún trae secciones de tienda (Cupones, Campañas, Parametrías, Talles, Cambios, Catálogo). Se ataca en P2/P3 con confirmación del dueño antes de borrar datos.
